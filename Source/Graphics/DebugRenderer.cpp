...
void DebugRenderer::Render()
{
	...
	// 定数バッファ設定
	graphics.GetDeviceContext()->VSSetConstantBuffers(0, 1, constantBuffer.GetAddressOf());
	//context->PSSetConstantBuffers(0, 1, constantBuffer.GetAddressOf());

	// レンダーステート設定
	const float blendFactor[4] = { 1.0f, 1.0f, 1.0f, 1.0f };
	graphics.GetDeviceContext()->OMSetBlendState(blendState.Get(), blendFactor, 0xFFFFFFFF);
	graphics.GetDeviceContext()->OMSetDepthStencilState(depthStencilState.Get(), 0);
	graphics.GetDeviceContext()->RSSetState(rasterizerState.Get());
	...
	// 球描画
	graphics.GetDeviceContext()->IASetVertexBuffers(0, 1, sphereVertexBuffer.GetAddressOf(), &stride, &offset);
	for (const Sphere& sphere : spheres)
	{
		...
		graphics.GetDeviceContext()->UpdateSubresource(constantBuffer.Get(), 0, 0, &cbMesh, 0, 0);
		graphics.GetDeviceContext()->Draw(sphereVertexCount, 0);
	}
	spheres.clear();

	// 円柱描画
	graphics.GetDeviceContext()->IASetVertexBuffers(0, 1, cylinderVertexBuffer.GetAddressOf(), &stride, &offset);
	for (const Cylinder& cylinder : cylinders)
	{
		...
		graphics.GetDeviceContext()->UpdateSubresource(constantBuffer.Get(), 0, 0, &cbMesh, 0, 0);
		graphics.GetDeviceContext()->Draw(cylinderVertexCount, 0);
	}
	cylinders.clear();
}
